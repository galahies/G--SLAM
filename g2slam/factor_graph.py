...
class FactorGraph:
    ...
    def add_factors(self, ii, jj, remove=False):
        ...
        if self.max_factors > 0 and self.ii.shape[0] + ii.shape[0] > self.max_factors                and self.corr is not None and remove:
        ...
        for step in (pbar := trange(steps)):
            ...
            for i in range(0, self.jj.max()+1, s):
                ...
                with torch.cuda.amp.autocast(enabled=True):
                    ...
                    net, delta, weight, damping, upmask =                        self.update_op(self.net[:,v], self.video.inps[None,iis], corr1, motn[:,v], iis, jjs)
