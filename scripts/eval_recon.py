...
def completion_ratio(gt_points, rec_points, dist_th=0.05):
    ...
    return np.mean((distances < dist_th).astype(float))
...
def check_proj(points, width, height, fx, fy, cx, cy, c2w):
    c2w = c2w.copy()
    c2w[:3, 1] *= -1.0
    c2w[:3, 2] *= -1.0
...
def calc_2d_metric(rec_meshfile, gt_meshfile, align=True, n_imgs=1000):
    ...
    for _ in tqdm(range(n_imgs)):
        while True:
            ...
            tmp[:3, :] = c2w
            c2w = tmp
            seen = check_proj(pc_unseen, width, height, fx, fy, cx, cy, c2w)
            if not seen:
                break
...
def eval_recon(rec_mesh, gt_mesh, eval_2d, eval_3d, align=True):
    ...
    try:
        ...
        if eval_2d:
            result_2d = calc_2d_metric(rec_mesh, gt_mesh, align=align, n_imgs=10)
...
if __name__ == "__main__":
    ...
    parser.add_argument("rec_mesh", type=str, help="Reconstructed mesh path.")
    parser.add_argument("gt_mesh", type=str, help="Ground-truth mesh path.")
